...
public class ModelController : MonoBehaviour
{
    ...
    public static ModelController Create(
        GameObject host,
        Timer time,
        ModelContent modelContent) // YENİ: gereksiz bool parametresi kaldırıldı
    {
        var ctrl = host.AddComponent<ModelController>();
        ctrl.Init(time, modelContent);
        return ctrl;
    }

    public void Init(Timer time, ModelContent modelContent)
    {
        timer = time;
        mc = modelContent;
        started = false;
    ...
    private void Update()
    {
        if (!timer || !isPlaying) return; // YENİ: sadece playing'de çalış

...
public class ModelContent : Content
{
    ...
    public void ResetAnimation()
    {
        ...
        {
            _animator.enabled = false;
            _animator.speed = 1f; // Hızı normale döndür
        }
    }
    ...
    /// <summary>
    /// ModelContent için sadeleştirilmiş Constructor.
    /// </summary>
    public ModelContent(GameObject modelPrefab, float startingPoint, float duration)
    {
        ...
        _modelInstance.layer = 8; // Model katmanı
        SetLayerRecursively(_modelInstance, 8);

        // Modelin başlangıç pozisyonunu ve ayarlarını yap
    ...
    {
        // Döngüsel olmayan animasyon bittiğinde modeli gizle
    ...
    /// <summary>
    /// Modeli sahnede konumlandırmak için TimelineGrid'den bağımsız hale getirilmiş metod.
    /// </summary>
    private void PositionModelInScene()
    {
        // ViewerCore'u bularak onun görüntü alanını referans al
        ...
        if (viewerCore != null && viewerCore.displayArea != null)
        {
            ...
            // Modelin kameranın önünde görünmesi için Z değerini ayarla
            worldCenter.z = -80f;
            _modelInstance.transform.position = worldCenter;
        }
        else
        {
            // Fallback: Eğer bulunamazsa sahnenin merkezine koy
